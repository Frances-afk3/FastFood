...
export const registerUtenti = async (req, res) => {
  ...
  const existing = await db.collection("_Utenti").findOne({ email });
  if (existing) {
    return res.status(409).json({ error: "Email già registrata" });
  }

  const hashed = await bcrypt.hash(password, 10);
  const result = await db.collection("_Utenti").insertOne({ nome, email, password: hashed });

  res.status(201).json({ message: "Registrazione completata", id: result.insertedId });
};

export const loginUtenti = async (req, res) => {
  const { email, password } = req.body;

  const ristorante = await db.collection("_Utenti").findOne({ email });
